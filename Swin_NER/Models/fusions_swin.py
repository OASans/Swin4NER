...
class OriginTransformerEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.pos_encoder = AbsolutePositionalEncoding(config.hidden_size, config.dropout)
        encoder_layers = TransformerEncoderLayer(d_model=config.hidden_size, nhead=config.num_heads, dropout=config.dropout)
        self.transformer_encoder = TransformerEncoder(encoder_layers, config.num_layers)

    def forward(self, src: Tensor, src_mask: Tensor) -> Tensor:
        src = src.permute(1, 0, 2)  # shape [seq_len, batch_size, embedding_dim]
        # src_mask = src_mask.float()
        # src_mask = (1 - torch.matmul(src_mask, src_mask.permute(0, 2, 1))).bool()
        src_mask = (1 - src_mask).bool()
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, config, depths=[1, 1, 3, 1], num_heads=[3, 6, 12, 24],
        ...
        for i_layer in range(self.num_layers):
            layer = SwinTransformerLayer(seq_len=self.seq_len, dim=self.embed_dim,
    ...
    def forward_features(self, x, x_mask=None):
    ...
    def forward(self, x, x_mask=None):
        x = self.forward_features(x, x_mask)
        # x = self.head(x)
        return x 
